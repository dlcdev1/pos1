...
func main() {
	db, err := sql.Open("mysql", "sa:sasa@tcp(localhost:3306)/goexpert")
	if err != nil {
		panic(err)
	}
	defer db.Close()
...
func updateProduct(db *sql.DB, produtc *Product) error {
	...
	defer stmt.Close()
	_, err = stmt.Exec(produtc.Name, produtc.Price, produtc.ID)
	if err != nil {
		return err
	}
	return nil
